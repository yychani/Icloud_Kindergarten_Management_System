...
/**
 * Servlet implementation class SearchBanBoardServlet
 */
@WebServlet("/searchBan.bo")
public class SearchBanBoardServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public SearchBanBoardServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if((loginUser).getUType().equals("교사")){
			page="views/teacher/tcClassNoticeSearchPage.jsp";
		}else if((loginUser).getUType().equals("학부모")) {
			page="views/parents/newsClassNoticeSearchPage.jsp";
		}
		
		System.out.println(list.size());
			for(int i=0; i<list.size(); i++) {
				System.out.println(list.get(i).getTtitle());
			}
