...
/**
 * Servlet implementation class InsertTctcBoardServlet
 */
@WebServlet("/insertTctc.bo")
public class InsertTctcBoardServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public InsertTctcBoardServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			int result = new BoardService().insertTctcBoard(hmap,b);
			
			String page = "";
			if(result >0) {
				response.sendRedirect(request.getContextPath()+"/selectAllTctc.bo");
			}else {
				request.setAttribute("msg", "선생님 게시판 등록 실패");
