...
/**
 * Servlet implementation class InsertChildImgServlet
 */
@WebServlet("/insertChildImg.tbo")
public class InsertChildImgServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public InsertChildImgServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if(ServletFileUpload.isMultipartContent(request)) {
			...
			int result = new BoardService().insertChildImg(hmap,b);
			
			String page = "";
			
			if(result>0) {
				response.sendRedirect(request.getContextPath()+"/selectListChImg.tbo");
			}else {
				request.setAttribute("msg", "앨범 등록 실패");
